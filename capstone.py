...
range_model, range_model_file = load_model(
    [
        "range_model.pkl",
        "range_model.joblib",
        "ev_range_model.pkl",
...
battery_model, battery_model_file = load_model(
    [
        "battery_model.pkl",
...
def estimate_current_load(
    ...
    """ 
            Estimate electrical battery current automatically. 
    ...
    if vehicle_type == "Electric Scooter":
        ...
        weight_factor = (
            max(weight - 10000, 0)/ 10000 * 15
        )

    else:

        weight_factor = (
            max(weight - 1000, 0) / 1000* 4
        )
...
Smart Range Estimator
</div>

<div class="feature-text">
Estimate practical driving range using battery,
speed, traffic, terrain, temperature and driving style.
</div>
...
🤖 <b>Smart Automatic Estimation</b>
...
""",
    ...
    if st.button(
        ...
        estimated_current, estimated_power = (
            estimate_current_load(
                vehicle_type,
                weight,
                speed,
                soc,
        ...
        st.session_state.vehicle_weight = (
            weight
        )

        st.session_state.estimated_battery_temperature = (
            estimated_temperature
        )

        st.session_state.estimated_current_load = (
            estimated_current
        )

        st.session_state.estimated_power = (
            estimated_power
        )

        st.session_state.battery_voltage = (
            defaults["voltage"]
        )

        st.session_state.temperature_model_status = (
...
""",
        ...
        st.markdown(
            '<div class="section-title">'
            '⚡ Automatic Electrical Load Analysis'
            '</div>',
            unsafe_allow_html=True
        )
...
<b>Estimated Electrical Power:</b>
{estimated_power:.1f} kW

<br>

<b>Estimated Battery Current:</b>
...
🟢 <b>Normal Battery Temperature</b>

<br>

Estimated battery temperature:
<b>{estimated_temperature:.1f}°C</b>
...
🟡 <b>Elevated Battery Temperature</b>
...
<br>
Consider reducing aggressive driving
...
""",
    ...
    if st.button(
        ...
        st.session_state.battery_degradation = (
            degradation
        )

        st.session_state.battery_health_status = (
            health_status
        )

        st.session_state.battery_model_status = (
            "Calculated"
        )
...
<div class="metric-value"
style="font-size:{18 if metric[0] == "🤖 Source" else 29}px;">
{metric[1]}
</div>
...
🟡 <b>Battery Health Requires Monitoring</b>
...
🔴 <b>Battery Health Needs Attention</b>
...
# ============================================================
    ...
    st.write(
        "Compare the running cost of an EV with a petrol vehicle."
    )
...
""",
        ...
        default_consumption = (
            ...
            if st.session_state.consumption
            is not None
            else 15.0
        )
...
The current assumptions do not show
an EV running-cost advantage.
...
# ============================================================
    ...
    st.markdown(
        '<div class="hero-title">'
        '📊 Smart EV Intelligence Dashboard'
        '</div>',
        unsafe_allow_html=True
    )
...
⚡ <b>No EV analysis available yet.</b>
...
Go to <b>⚡ Range</b> and click
<b>Estimate My Range</b>.
...
""",
        ...
        st.markdown(
            '<div class="section-title">'
            '🔋 Electrical & Thermal Intelligence'
            '</div>',
            unsafe_allow_html=True
        )
...
""",
        ...
        with right:
            ...
            if estimated_temperature is not None:
                ...
                st.info(
                    "Battery temperature is not available yet."
                )
...
🔋 <b>Battery health has not been analysed yet.</b>

<br><br>

Go to <b>🔋 Battery</b> and run
<b>Estimate Temperature & Analyze Battery</b>
to add battery-health information to this dashboard.
...
""",
        ...
        dashboard_data = pd.DataFrame({
            ...
            "Value": [
                ...
                (
                    f"{battery_degradation:.2f}%"
                    if battery_degradation is not None
                    else "Not analysed"
                ),

                (
                    temperature_model_status
                    if temperature_model_status
                    else "Not available"
                )
            ]
        })
        ...
        if soc < 20:
            ...
            recommendations.append(
                ...
                "Consider charging before continuing a long trip."
            )

        elif soc < 40:

            recommendations.append(
                "🟡 Battery charge is moderate. "
                "Plan your next charging session."
            )
        ...
        if consumption > 25:
            ...
            recommendations.append(
                "⚠️ Energy consumption is relatively high. "
                "Eco driving and lower speeds may improve range."
            )
        ...
        if estimated_temperature is not None:
            ...
            if estimated_temperature >= 50:
                ...
                recommendations.append(
                    ...
                    "Reduce aggressive driving and electrical load."
                )
        ...
        if battery_health is not None:
            ...
            if battery_health < 70:
                ...
                recommendations.append(
                    ...
                    "Consider professional battery inspection."
                )

            elif battery_health < 80:

                recommendations.append(
                    "🟡 Battery health is moderate. "
                    "Monitor charging and thermal behaviour."
                )
...
<h2>
⚡ Smart EV Range & Dynamic Battery
Degradation Estimator
</h2>

<p>

Smart EV AI is an intelligent electric vehicle
analytics platform designed to estimate real-world
range, analyse battery health and compare EV
running costs with petrol vehicles.
...
The application supports trained machine-learning
models for EV prediction tasks including battery
...
Users do not need to know the battery current.
The application automatically estimates electrical
load using vehicle type, vehicle weight, speed,
SOC, driving style, terrain, passenger load,
...
Battery temperature is estimated from operating
conditions instead of requiring the user to manually
enter internal battery temperature.

</p>


<h3>📊 Important Input Parameters</h3>
...
<li>Passenger Load</li>

<li>Climate Control Usage</li>
...
Intelligent EV Range • Battery Health •
Automatic Load • Temperature Analytics
